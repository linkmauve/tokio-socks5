...
impl Client {
    ...
    ///
    /// As a side note, you'll notice a number of `Box::new()` annotations here
    /// to box up intermediate futures. From a library perspective, this is not
    /// necessary, but without them the compiler is pessimistically slow!
    /// Essentially, the `Box::new()` annotations here improve compile times,
